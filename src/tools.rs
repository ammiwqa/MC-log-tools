...
pub fn format_filename_error(filename: &str, error: &FilenameError) -> String {
    match error {
        FilenameError::Empty =>
            "DB name cant be empty".to_string(),

        FilenameError::ForbiddenChar(c) =>
            format!("DB name '{}' contains forbidden char '{}'.\n\
                     You can use: letters, numbers, spaces, hyphens, underscores and dots.\n\
                     Forbidden: < > : \" / \\ | ? *", filename, c),

        FilenameError::EndsWithSpaceOrDot =>
            format!("BD name '{}' cannot end with a space or a dot", filename),

        FilenameError::ReservedName(name) =>
            format!("DB name '{}' is a reserved Windows device name.\n\
					Reserved names: CON, PRN, AUX, NUL, COM1-COM9, LPT1-LPT9", name),

        FilenameError::TooLong { max, actual } =>
...
pub fn file_modified_days_local(path: &str) -> std::io::Result<i64> {
    ...
    // обрезаем до полуночи локального времени
    let date = dt.date_naive();

    // переводим в unix days
